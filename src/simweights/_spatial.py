...
class CylinderBase:
    ...
    def projected_area(self, cos_zen: ArrayLike) -> NDArray[np.float64]:
        """
        Returns the cross sectional area of a cylinder in cm^2
...
class UniformSolidAngleCylinder(CylinderBase):
    r"""A Cylinder where the the angular distribution was sampled as if it were uniform on
...
class NaturalRateCylinder(CylinderBase):
    r"""This is the angular distribution used where the primary zenith distribution matched the natural rate
    of an isotropic source incident on a cylinder.
...
class CircleInjector:
    """This represents the spatial distribution used by older neutrino-generator versions
