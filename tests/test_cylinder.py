...
class TestCylinder(unittest.TestCase):
    def check_diff_etendue(self, c, l, r):
        ...
        self.assertAlmostEqual(c.projected_area(-0.5), np.pi * r ** 2 / 2 + 3 ** 0.5 * l * r)
        self.assertAlmostEqual(c.projected_area(-1 / 2 ** 0.5), r / 2 ** 0.5 * (np.pi * r + 2 * l))
        ...
        self.assertAlmostEqual(
            c._diff_etendue(-0.5), -np.pi ** 2 / 4 * r * (r + 2 / 3 * l * (3 ** 1.5 / np.pi + 8))
        )
        self.assertAlmostEqual(
            c._diff_etendue(-(0.5 ** 0.5)), -np.pi ** 2 / 2 * r * (r + l * (2 / np.pi + 3))
        )
        self.assertAlmostEqual(c._diff_etendue(0), -np.pi ** 2 * l * r)
        self.assertAlmostEqual(c._diff_etendue(0.5 ** 0.5), np.pi ** 2 / 2 * r * (r + l * (2 / np.pi - 1)))
        self.assertAlmostEqual(
            c._diff_etendue(0.5), np.pi ** 2 / 4 * r * (r + 2 / 3 * l * (3 ** 1.5 / np.pi - 4))
        )
    ...
    def test_cylinder_base(self):
        ...
        with self.assertRaises(ValueError):
            CylinderBase(l, r, -1.5, 0.5)
        with self.assertRaises(ValueError):
            CylinderBase(l, r, -0.5, 1.5)
        with self.assertRaises(ValueError):
            CylinderBase(l, r, 0.5, -0.5)
