...
from icecube.weighting.weighting import ParticleType
...
ParticleType.from_pdg = classmethod(lambda cls, i: getattr(cls, PDGCode[i].name))





from . import units as I3Units
...
class FixedFractionFlux(object):
    ...
    """
    corsika_to_pdg = numexpr.NumExpr(build_lookup([(int(ParticleType.from_pdg(v)), v) for v in PDGCode.__members__.keys()]))
