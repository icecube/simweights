...
class CylinderBase:
    def __init__(self,length,radius,a,b):
        if a < -1 or b > 1:
            raise ValueError(self.__class__.__name__+": a must be >= -1 and b must be <= 1")
        if a >= b:
            raise ValueError(self.__class__.__name__+": a must be < b")
        self.length  = length
        self.radius  = radius
        self.a       = a
        self.b       = b
    ...
    def projected_area(self,x):
        assert np.all(x >=-1)
        assert np.all(x <=+1)
        return self._cap*abs(x) + self._side*np.sqrt(1-x**2)
