...
class NuGenWeighter(Weighter):
    ...
    def get_surface(self, cylinder_height, cylinder_radius, max_energy,
                    max_zenith, min_energy, min_zenith, n_events,
                    power_law_index, primary_type, **kwargs):
        surface = VolumeCorrCylinder(cylinder_height, cylinder_radius,
                                     np.cos(max_zenith),np.cos(min_zenith))
        assert(power_law_index>0)
        powerlaw = PowerLaw(-power_law_index, min_energy, max_energy)
        probs = [GenerationSurface(primary_type, 0.5 * n_events, powerlaw,surface),
    ...
    def get_surface_params(self):
        return dict(energy = self.get_column("I3MCWeightDict", 'PrimaryNeutrinoEnergy'),
